...
GetOptions(
		...
		'samtoolsPath'=>\our$samtoolsPath,
		'wigglePath'=>\our$wigglePath
		);
my$help=<<"HELP";
	--GffFile:	<str>	Gff format gene annotation file input
	--JuncFile:	<str>	Bed format junction annotation file input, optional
	...
	--samtoolsPath: <str> samtoolsPath
	--wigglePath: <str> wiggle path from topHat
HELP
...
if(!$samtoolsPath){$samtoolsPath="/hwfssz1/ST_EARTH/Reference/ST_DIVERSITY/PUB/USER/zhangpei/bin/samtools-0.1.18"}
if(!$wigglePath){$wigglePath="/hwfssz1/ST_EARTH/Reference/ST_DIVERSITY/PUB/USER/zhangpei/bin/tophat-1.3.1/bin"}

$ENV{'PATH'}=$ENV{'PATH'}.":".$samtoolsPath;
$ENV{'PATH'}=$ENV{'PATH'}.":".$wigglePath;
...
if($JuncFile){
	...
	while(<IN>){
		...
		my$start=$A[1];
		my$end=$A[2];
		$temp_junc{$A[0]}{$start}{$end}{$A[3]}=$A[4];
	}
	close IN;
...
sub sam{
	...
	my$end=shift;
	my$strand=shift;
	system "samtools view $BamFile $scaffold:$start-$end >temp.sam";
	if($Unique){
		open INx,"temp.sam";
		open OUTx,">temp.uniq.sam";
		while(<INx>){
		    chomp;
   			my@A=split(/\t/);
		    if($A[0] eq '@SQ' || $A[0] eq '@HD' || $A[0] eq '@PG'){print OUT "$_\n";next}
		    my$tag=0;
		    foreach my$ele(@A){
...
sub coverage{
	...
	system "wiggles temp.sam temp.wiggle";
...
sub overlap{
    ...
    if($d<=$b){
        $result[0]=$d-$c+1;
        $result[1]=$result[0]/$l1;
        $result[2]=$result[0]/$l2;
    }elsif($c<=$b){
        $result[0]=$b-$c+1;
        $result[1]=$result[0]/$l1;
        $result[2]=$result[0]/$l2;
