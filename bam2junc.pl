#!/usr/bin/perl -w
use strict;
if(!$ARGV[0]){die "perl $0 chr_list genome.fa alignment.bam samtools_path> junc.bed\n"}
$ENV{'PATH'}=$ENV{'PATH'}.":".$ARGV[3];
my%list;
open IN,"$ARGV[0]";
while(<IN>){
	chomp;
...
foreach $chr(keys %list){
	foreach my$start(keys %{$list{$chr}}){
		system "samtools view $ARGV[2] $chr:$start-$list{$chr}{$start} >$chr.sam";
		open IN,"$chr.sam";
		while(<IN>){
			chomp;
			my@A=split(/\t/);
...
#				print "$startChr\t$endChr\n";
				...
				$junc{$A[2]}{$start}{$end}{$strand}[3]=$type;
			}
		}
	}
	close IN;
...
my$JuncNum=0;
print "chr\tstart\tend\tstrand\tdepth\ttype\n";
	...
	foreach my$ele(@start){
		...
		foreach my$ele2(@end){
			foreach my$strand(keys %{$junc{$chr}{$ele}{$ele2}}){
				...
				print "$chr\t$ele\t$ele2\t$strand\t$junc{$chr}{$ele}{$ele2}{$strand}[2]\t$junc{$chr}{$ele}{$ele2}{$strand}[3]\n";
